...
export default function Dynmaic_Products_Shop() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`http://192.168.1.3/api/pet-shop-products?populate=*`);
        const fetchSliderData = await axios.get('http://192.168.1.3/api/bakery-sliders?populate=*');
